...
bibfile=RefManageR::ReadBib("C:/users/magb/Dropbox/Conferences-Talks/EMS2023/refs.bib",check = FALSE)
```
class: title-slide
...
DO WE WANT TO PUT SOMETHING ON SEMI-ECOLOGICAL STUDIES TOO?
...
`r vspace("10px")`
  - Temperature and COPD hospitalisations (semi-ecological study; space-time effect; non linear dose-response)
...
.pull-left[
`r include_fig("PopCoverage.png", width="50%", title="% pop coverage")`
.center[Percentage population coverage]
]

.pull-right[
`r include_fig("STW.png", width="30%", title="Location of STWs and their catchment areas")`
...
- Geospatial model where the latent mean concentration at monitoring site includes
1. Covariates
2. Regional effect
3. Temporal trend 
4. Spatio-temporal random effect, modelling the deviation from the national trend with correlation in space 
...
- The posterior predictive distribution is used to detect increases in viral concentration over two consecutive weeks
